...
module.exports = class TunnelCluster extends EventEmitter {
  ...
  open() {
    ...
    remote.on('data', data => {
      ...
      if (match) {
        const userAgentMatch = data.toString().match(/user-agent: (.+)/);
        if (userAgentMatch) {
          const userAgent = userAgentMatch[1];
          const botUserAgents = ['SemrushBot', 'SemrushBot/7~bl', 'dotbot', 'SiteAuditBot', 'SplitSignalBot', 'SemrushBot-BA', 'SemrushBot-SI', 'SemrushBot-SWA', 'SemrushBot-OCOB']; // TODO: move list to database or .env. 
          const isBot = botUserAgents.some(bot => userAgent.includes(bot));
          if (isBot) {
            const forbiddenResponse = 'HTTP/1.1 403 Forbidden\r\n\r\nForbidden';
            remote.write(forbiddenResponse);
